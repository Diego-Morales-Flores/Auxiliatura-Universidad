...
def validarCadena(cadena, estado, pila, cont):
    if cont>3:
        return bool(False)
    if cadena==''and pila=='':
        return bool(True)
    ...
    for i in range(len(D)):                                                     
        ...
        if izq[0]==estado and (izq[1]==letra or izq[1]=='') and izq[2]==topePila:
            ...
            for j in range(len(D[i][1])):
                ...
                if izq[1]=='':
                    cont=cont+1
                    recursion=validarCadena(cadena,D[i][1][j][0],pila,cont)
                else:
                    recursion=validarCadena(cadena[1:],D[i][1][j][0],pila,cont)
...
for i in range(len(combinaciones)):
    ...
    valido=validarCadena(combinaciones[i],'q0','Z',0)
